...
bool binarySearch(vector<ll> arra,ll arraySize, ll key){
    ...
    while(startPoint <= endPoint){
        ll midPoint = (startPoint + endPoint) / 2;
...
int main() { 
    ...
    for(ll j = 0; j < Prime.size(); j++){
        Tprime.push_back(Prime[j] * Prime[j]);
    }

    for(ll i = 0; i < n; i++){
        cin >> x;
        if( binarySearch(Tprime, Tprime.size(), x)){
            cout << "YES" << endl; 
        }else
        {
            cout << "NO" << endl;
